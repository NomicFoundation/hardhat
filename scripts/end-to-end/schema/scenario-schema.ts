import type { CloneScenarioDefinition, ScenarioDefinition } from "../types.ts";

export function isScenarioDefinition(
  value: unknown,
): value is ScenarioDefinition {
  if (typeof value !== "object" || value === null) {
    return false;
  }

  const obj = value as Record<string, unknown>;

  if (obj.type === "clone") {
    return isCloneScenarioDefinition(value);
  }

  return false;
}

export function isCloneScenarioDefinition(
  value: unknown,
): value is CloneScenarioDefinition {
  if (typeof value !== "object" || value === null) {
    return false;
  }

  const obj = value as Record<string, unknown>;

  return (
    typeof obj.repo === "string" &&
    typeof obj.commit === "string" &&
    obj.packageManager === "npm" &&
    Array.isArray(obj.tags) &&
    obj.tags.every((t: unknown) => typeof t === "string") &&
    (obj.commands === undefined ||
      (Array.isArray(obj.commands) &&
        obj.commands.every((c: unknown) => typeof c === "string"))) &&
    (obj.env === undefined || isStringRecord(obj.env)) &&
    (obj.preinstall === undefined || typeof obj.preinstall === "string") &&
    (obj.install === undefined || typeof obj.install === "string") &&
    (obj.submodules === undefined || typeof obj.submodules === "boolean")
  );
}

function isStringRecord(value: unknown): value is Record<string, string> {
  if (typeof value !== "object" || value === null) {
    return false;
  }

  return Object.values(value as Record<string, unknown>).every(
    (v) => typeof v === "string",
  );
}
