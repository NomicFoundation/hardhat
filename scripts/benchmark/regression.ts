...
DESCRIPTION
  ...
  once and subtracted, like hyperfine's calibration) and CPU timed by
  bash's \`time\` builtin.
  ...
  every 100 ms via /proc, tracking each process's peak RSS (the kernel's
  VmHWM high-water mark — the counter behind GNU time's %M; a process
  shorter than the sampling interval can be missed). This is emitted as a
  separate "<scenarioId> / <name> (peak RSS)" entry (unit
  MB): its value is the highest single-process peak observed across runs,
  ...
  try {
    ...
    const measured = await runSeries(
      ...
      path.join(scenarioTmpDir, `${slugify(name)}-cpu.txt`),
      {
        ...
        runs: cfg.runs,
        prepare: cfg.prepare,
        onRun: (run, i) => log(`  run ${i + 1}/${cfg.runs}: ${formatRun(run)}`),
      },
    );
    ...
    for (const stepName of stepNames) {
      ...
      try {
        if (stepRuns !== undefined) {
          const measured = await runMeasured(step.command, timingPath, {
            cwd: workingDir,
            env,
          });
