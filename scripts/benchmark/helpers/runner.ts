...
export interface SeriesOptions extends RunOptions {
  ...
  onWarmup?: (index: number, total: number) => void;
  /** Called after each completed measured run. */
  onRun?: (run: MeasuredRun, index: number, total: number) => void;
}
...
export async function runSeries(
  ...
  const { runs, warmup = 0, prepare, onWarmup, onRun, ...runOptions } = options;
  ...
  for (let i = 0; i < runs; i++) {
    ...
    const result = await runMeasured(command, timingPath, runOptions);
    results.push(result);
    onRun?.(result, i, runs);
  }
