...
- **aave-v4 was the blocker, and reducing CV was far cheaper than adding runs.** Cutting its fuzz iterations (1000 → 100) dropped per-run time ~7× (82 s → 11.8 s) and its p95 CV from 9.7% to ~5.1%, so its 5% count fell from ~85 runs (~116 min) to **24 runs (~4.7 min)**. This is exactly the `runs ∝ CV²` lever — fewer fuzz iterations — with the runtime cut compounding it. It is set to **24 runs**, the 5% count: σ ≈ √2·5.1/√24 ≈ 1.49%, right at the 1.5% target. (The 5.1% CV is provisional, from 2 post-change commits, so this sits essentially on the threshold; re-confirm as history accrues.)
