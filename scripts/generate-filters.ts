...
function main() {
  ...
  const pnpmLockfilePath = path.join(
    import.meta.dirname,
    "..",
    "pnpm-lock.json",
  );
  ...
  const internalDependenciesMap: Record<string, string[]> = {};
  for (const [pkg, allDependencies] of Object.entries(
    pnpmLockfile.importers as Record<
      string,
    ...
    const internalDependencies = Object.values(allDependencies)
      ...
      .map((version) => path.join(pkg, version));
  ...
  for (const [pkg, dependencies] of Object.entries(internalDependenciesMap)) {
    ...
    const packageFilters = [pkg, ...dependencies].map((dependency) =>
