...
# npm OpenZeppelin) already carry range pragmas and need no patching. Use
# node for the file transforms to avoid BSD/GNU sed portability issues
...
if (patched === 0) {
  console.error(
    'lidofinance-vaults-solx preinstall: no \`' + FROM + '\` pragmas found — ' +
...
console.log('lidofinance-vaults-solx preinstall: relaxed the pinned pragma in ' + patched + ' files');
"
