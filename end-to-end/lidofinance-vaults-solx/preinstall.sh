...
# 0.8.34. The walker also covers exact-0.8.25 files outside contracts/0.8.25:
# contracts/upgrade needs it for the upgrade-tree cells (which compile that
# tree at 0.8.34 — see hardhat.config.solx.ts), and the rest (tooling, tests)
# is harmless, since the wrapper config's scoped sources and --no-tests keep
# it out of every cell. The transitive imports (contracts/common, vendored +
# npm OpenZeppelin) already carry range pragmas and need no patching. Unlike
# aave, lib/ must NOT be skipped: the repo has no submodules, and
# contracts/0.8.25/vaults/lib holds first-party sources whose exact pragmas
# need relaxing too.
node "$MONOREPO_ROOT/scripts/benchmark/relax-pragmas.ts" --scenario lidofinance-vaults-solx --from 0.8.25
...
if (ethersVersion === undefined) {
  console.error(
    'lidofinance-vaults-solx preinstall: expected an ethers pin in package.json — ' +
...
if [ -e foundry.toml ]; then
  echo "lidofinance-vaults-solx preinstall: the pinned commit ships a foundry.toml — the pin may have changed; refusing to overwrite it." >&2
  exit 1
fi
