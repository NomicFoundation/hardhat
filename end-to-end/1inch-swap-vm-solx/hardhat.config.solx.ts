...
// settings; the legacy cells explicitly flip `viaIR` to false. The Yul
// optimizerSteps ride along in every cell: solc honors them on the via-IR
// pipeline, solx parses and ignores them (its LLVM pipeline replaces Yul
// optimization) — each compiler measured at its realistic pipeline.
