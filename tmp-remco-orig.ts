...
// type LineStatus = "covered" | "uncovered";
...
//   const minChar = Math.min(...nodes.map((n) => n.startUtf16));
//   const maxChar = Math.max(...nodes.map((n) => n.endUtf16));

//   const result = new Map<number, LineStatus>();

//   for (let char = minChar; char <= maxChar; char++) {
//     const nodesCoveringChar = nodes.filter(
//       (n) => n.startUtf16 <= char && n.endUtf16 >= char
//     );
//     if (nodesCoveringChar.length === 0) {
//       continue;
//     }

//     const nodesCoveringCharWithLength = nodesCoveringChar.map((n) => ({
//       node: n,
//       length: n.endUtf16 - n.startUtf16 + 1,
//       hit: hitSet.has(n.tag),
//     }));

//     const minLength = Math.min(
//       ...nodesCoveringCharWithLength.map((c) => c.length)
//     );
//     const mostSpecific = nodesCoveringCharWithLength.filter(
//       (c) => c.length === minLength
//     );

//     const anySpecificHit = mostSpecific.some((c) => c.hit);
//     // const anySpecificMiss = mostSpecific.some((c) => !c.hit);

//     if (anySpecificHit) {
//       result.set(char, "covered");
//     } else {
//       result.set(char, "uncovered");
//     }
//   }
...
//   Array.from(result.entries()).forEach(([char, status]) => {
//     console.log(`${fileContent[char]}: ${status}`);
//   });
...
//     computeLineCoverage(this.metadata, allExecutedTags);
