...
export default function () {
  ...
  internalTask(TASK_COMPILE_GET_COMPILER_INPUT, async (_, { config, run }) => {
    ...
    return getInputFromDependencyGraph(
      dependencyGraph,
      (config as any).solc.optimizer,
      (config as any).solc.evmVersion
    );
  });
  ...
  internalTask(TASK_COMPILE_COMPILE, async (_, { config, run }) => {
    ...
    await cacheBuidlerConfig(config.paths, (config as any).solc);
  ...
  task("compile:legacy", "Compiles the entire project, building all artifacts")
