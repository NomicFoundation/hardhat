...
describe("Flatten task", () => {
  ...
  describe("Add license", function () {
    ...
    it("Should add a license", async function () {
      const aFlattened = await this.env.run(TASK_FLATTEN_GET_FLATTENED_SOURCE, {
        files: ["contracts/AWithLicence.sol"], license: 'A-LICENSE'
      })

      assert.isTrue(aFlattened.includes("// SPDX-License-Identifier: A-LICENSE"))

      let abFlattened = await this.env.run(TASK_FLATTEN_GET_FLATTENED_SOURCE, {
        files: ["contracts/BWithLicence.sol", "contracts/AWithLicence.sol"], license: 'A-LICENSE'
      })

      assert.isTrue(abFlattened.includes("// SPDX-License-Identifier: A-LICENSE"))

      // Replace match
      abFlattened = abFlattened.replace("// SPDX-License-Identifier: A-LICENSE", '')

      assert.isFalse(abFlattened.includes("// SPDX-License-Identifier: A-LICENSE"))
    })
  })
