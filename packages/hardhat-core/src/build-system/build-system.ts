...
import { getSolidityFilesCachePath } from "./util/solidity-files-cache";
import { getFullyQualifiedName } from "./util/contract-names";
// --------------END PACKAGE
...
export async function taskCompileSolidityCompileJob(
  ...
  const input: CompilerInput = await taskCompileSolidityGetCompilerInput(
    compilationJob
  );
