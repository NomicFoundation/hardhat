...
import { network } from "../../lib/hardhat-lib";
...
{
  private _addedCompilationResults: [string, CompilerInput, CompilerOutput][] =
    [];
  ...
  private async _reset(networkConfig?: RpcHardhatNetworkConfig) {
    ...
    for (const [solcVersion, input, output] of this._addedCompilationResults) {
      // This should succeed as only succesful compilation results are added
