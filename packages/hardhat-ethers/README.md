...
By default, `HardhatEthersSigner.sendTransaction` follows ethers.js and resolves as soon as the submitted transaction is available from the network. If you want a network connection to wait until each submitted transaction has a receipt before resolving, enable `waitForTransactionReceipts` in that network's `ethers` config:
