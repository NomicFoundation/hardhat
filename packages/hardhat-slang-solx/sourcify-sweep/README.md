...
- `..`-prefixed virtual paths (parent-relative verification layouts) are re-rooted N synthetic levels deeper (N = the deepest leading-`..` run), so they cannot escape the fixture tree; relative-import arithmetic survives because every path — remapping targets included — shifts uniformly.
- Non-lowercase `.sol` extensions (`Token.SOL`) are renamed to `.sol`; Hardhat's source discovery only picks up `.sol` files, while solc accepted any extension.
- A build that exits 0 without producing any build info counts as `harness-fail`, not `ok`: it means the fixture had no discoverable sources.
- The `slangSolx` profile receives `evmVersion`, `libraries` and `viaIR` (which selects solx's Yul pipeline — via-IR contracts can rely on IR-only features), but not solc's optimizer settings: those don't map onto solx's LLVM -O modes, so the plugin default applies. The solc baseline profile gets the original settings.
