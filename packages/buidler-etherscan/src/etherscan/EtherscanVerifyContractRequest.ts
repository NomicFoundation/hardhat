...
export default class EtherscanVerifyContractRequest {
    ...
    constructor(
        ...
        this.constructorArguements = constructorArguments ? constructorArguments.join(' ') : '';
    ...
    private setLibraries(libraries: string) {
        ...
        try {
            if(libraries) parsedLibraries = JSON.parse(libraries);
        } catch (e) {
            throw new BuidlerPluginError('Failed to parse libraries. Reason: ' + e.message);
        }
