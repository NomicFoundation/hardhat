...
describe("Plugin integration tests", function() {
  ...
  it("Test verifying deployed contract on etherscan", async () => {
    let flattenedSource: string = await env.run(TASK_FLATTEN_GET_FLATTENED_SOURCE);
    ...
    const compilationResult = await new ContractCompiler(env.run).compile(flattenedSource, "TestContract1");
    compilationResult.bytecode = linker.linkBytecode(compilationResult.bytecode,
      {
