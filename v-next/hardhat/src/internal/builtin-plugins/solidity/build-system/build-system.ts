...
export class SolidityBuildSystemImplementation implements SolidityBuildSystem {
  ...
  readonly #defaultConcurrenty = os.cpus().length;
  ...
  public async build(
    ...
    const isSuccessfulBuild = results.every(
      (result) =>
        result.errors?.every((error) => error.severity !== "error") ?? true,
    );
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      const errors = await Promise.all(
        (result.errors ?? []).map((error) =>
          this.remapCompilerError(compilationJob, error),
        ),
      );
  ...
  public async runCompilationJob(
    ...
    log(
      `Compiling ${numberOfFiles} files with solc ${compilationJob.solcConfig.version}`,
    );
  ...
  public async remapCompilerError(
    ...
    shouldShortenPath: boolean = false,
    ...
    return {
      ...
      formattedMessage: error.formattedMessage?.replace(
        ...
        (_match, prefix, sourceName) => {
          ...
          const replacement = shouldShortenPath
  ...
  async #downloadConfiguredCompilers(): Promise<void> {
    ...
    await downloadConfiguredCompilers(this.#getAllCompilerVersions());
