...
export async function ethereumsjsHardforkToEdrSpecId(
  hardfork: HardforkName,
): Promise<SpecId> {
  const { SpecId } = await import("@nomicfoundation/edr");
...
export async function edrSpecIdToEthereumHardfork(
  specId: SpecId,
...
export async function ethereumjsMempoolOrderToEdrMineOrdering(
  mempoolOrder: MempoolOrder,
): Promise<MineOrdering> {
  const { MineOrdering } = await import("@nomicfoundation/edr");
