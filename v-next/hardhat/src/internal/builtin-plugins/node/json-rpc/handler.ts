...
const log = debug("hardhat:core:tasks:node:json-rpc:handler");
...
export class JsonRpcHandler {
  ...
  public handleHttp = async (
    ...
    log(`Handling HTTP request: ${req.method} ${req.url}`);
  ...
  public handleWs = async (ws: WebSocket): Promise<void> => {
    log("Handling WebSocket connection");
    ...
    ws.on("message", async (msg: string) => {
      log(`Handling WebSocket message: ${msg}`);
  ...
  #sendResponse(
    ...
    log(`Sending response: ${JSON.stringify(rpcResp, undefined, 2)}`);
  ...
  async #handleSingleRequest(req: JsonRpcRequest): Promise<JsonRpcResponse> {
    log(`Handling single request: ${JSON.stringify(req, undefined, 2)}`);
  ...
  // NOTE: In v2, the subscriptions were strings
  async #handleSingleWsRequest(
    rpcReq: JsonRpcRequest,
    subscriptions: unknown[],
  ) {
    log(`Handling WebSocket request: ${JSON.stringify(rpcReq, undefined, 2)}`);
  ...
  readonly #handleRequest = async (
    ...
    log(`Handling request: ${JSON.stringify(req, undefined, 2)}`);
