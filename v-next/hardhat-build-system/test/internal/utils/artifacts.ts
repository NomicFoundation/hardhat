...
describe("Artifacts class", function () {
  ...
  describe("Artifacts reading and saving", function () {
    const getTmpDir = useTmpDir("artifacts");
    ...
    it("Should save the artifact even if the artifacts directory doesn't exist", async function () {
      const nonexistentPath = path.join(getTmpDir(), "I-DONT-EXIST");
      const name = "Lib";
      const output = COMPILER_OUTPUTS.Lib;
      const artifact = getArtifactFromContractOutput(
        "source.sol",
    ...
    it("Should be possible to get an absolute path to an artifact given a fully qualified name", async function () {
      ...
      assert.equal(artifactPath.startsWith(getTmpDir()), true);
      assert.equal(artifactPath.endsWith(".json"), true);
    });
    ...
    it("Trying to get a build info of an artifact that doesn't have one should just return undefined (async)", async function () {
      ...
      assert.equal(storedBuildInfo, undefined);
    });
    ...
    it("Should be possible to get the paths to all the artifacts, debug files and build infos", async function () {
      ...
      assert.equal(artifactPaths.length, Object.keys(COMPILER_OUTPUTS).length);
      assert.equal(
        artifactPaths.every((p) => p.startsWith(getTmpDir())),
        true,
      );
      assert.equal(
        artifactPaths.every((p) => p.endsWith(".json")),
        true,
      );
      assert.equal(debugFilePaths.length, Object.keys(COMPILER_OUTPUTS).length);
      assert.equal(
        debugFilePaths.every((p) => p.startsWith(getTmpDir())),
        true,
      );
      assert.equal(
      ...
      assert.equal(buildInfoPaths.length, 1);
      assert.equal(
        buildInfoPaths.every((p) => p.startsWith(getTmpDir())),
        true,
      );
      assert.equal(
        buildInfoPaths.every((p) => p.endsWith(".json")),
        true,
      );
    });
    ...
    it("Should be able to save build-infos with partial fields", async function () {
      ...
      >(solcOutput: OutputT) {
        ...
        const read: BuildInfo = await readJsonFile(buildInfoPath);
  ...
  describe("Caching", function () {
    ...
    beforeEach(async function () {
      artifacts = new Artifacts(getTmpDir());
    ...
    describe("Successful caching", function () {
      it("Should cache the path to each artifact and build info, and the lists of paths", async function () {
        assert.equal((await artifacts.getArtifactPaths()).length, 1);
        assert.equal((await artifacts.getDebugFilePaths()).length, 1);
        assert.equal((await artifacts.getBuildInfoPaths()).length, 1);

        // we create some other files, which shouldn't be returned
        await writeJsonFile(path.join(getTmpDir(), "c.sol", "B.json"), {});
        await writeJsonFile(path.join(getTmpDir(), "c.sol", "B.dbg.json"), {});
        await writeJsonFile(
          path.join(getTmpDir(), "build-info", "something.json"),
          {},
        );
    ...
    describe("Clear cache", function () {
      it("Should clear all the caches", async function () {
        ...
        assert.equal((await artifacts.getArtifactPaths()).length, 2);
        assert.equal((await artifacts.getDebugFilePaths()).length, 2);
        assert.equal((await artifacts.getBuildInfoPaths()).length, 2);
      });
    ...
    describe("Automatic cache clearing", function () {
      ...
      it("Shouldn't re-enable the cache", async function () {
        ...
        assert.equal((await artifacts.getArtifactPaths()).length, 3);
        assert.equal((await artifacts.getDebugFilePaths()).length, 3);
        assert.equal((await artifacts.getBuildInfoPaths()).length, 3);
      });
    });
  ...
  describe("artifactExists", function () {
    ...
    it("should check that the artifact exists", async function () {
      ...
      // FQN
      assert.equal(
        await artifacts.artifactExists("source.sol:Interface"),
        true,
      );
      // non-FQN
      assert.equal(await artifacts.artifactExists("Interface"), true);
    });
    ...
    it("should check that the artifact doesn't exist", async function () {
      ...
      // FQN, missing file
      assert.equal(await artifacts.artifactExists("invalid/path.sol:A"), false);
      // FQN, missing artifact
      assert.equal(await artifacts.artifactExists("source.sol:A"), false);
      // FQN, wrong casing
      assert.equal(
        await artifacts.artifactExists("source.sol:interface"),
        false,
      ...
      assert.equal(await artifacts.artifactExists("A"), false);
    });
  });
});
