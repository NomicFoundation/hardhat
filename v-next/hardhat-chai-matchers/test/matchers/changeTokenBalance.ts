    ...
    describe("transaction that transfers some tokens", () => {
      ...
      describe("assertion failures", () => {
        ...
        it("uses the token name if the contract doesn't have a symbol", async () => {
          const TokenWithOnlyName = await ethers.getContractFactory<[], Token>(
            "TokenWithOnlyName",
          );
          ...
          await expect(
            ...
            ).to.changeTokenBalance(provider, tokenWithOnlyName, receiver, 500),
          ).to.be.rejectedWith(
            AssertionError,
      ...
      describe(CHANGE_TOKEN_BALANCES_MATCHER, () => {
        ...
        it("arrays have different length", async () => {
          expect(() =>
            expect(
              mockToken.transfer(receiver.address, 50),
            ).to.changeTokenBalances(provider, mockToken, [sender], [-50, 50]),
          ).to.throw(
            Error,
