...
describe("BigNumber matchers", () => {
  ...
  describe("length", () => {
    ...
    describe("positive, successful assertions", () => {
        ...
        for (const convert of [
          ...
          describe(`with object ${obj.toString()} and with length operand of type ${typestr(
            ...
            for (const lenFunc of lengthFunctions) {
              it(`.to.have.${lenFunc} should work`, () => {
    ...
    describe("negative, successful assertions", () => {
      for (const { obj, len } of negativeSuccessCases) {
        for (const convert of [
          ...
          describe(`with object ${obj.toString()} and with length operand of type ${typestr(
            ...
            for (const lenFunc of lengthFunctions) {
              it(`should work with .not.to.have.${lenFunc}`, () => {
    ...
    describe("positive, failing assertions should throw the proper error message", () => {
      for (const { obj, len, msg } of positiveFailureCases) {
        for (const convert of [
          ...
          describe(`with object ${obj.toString()} and with operand of type ${typestr(
            ...
            for (const lenFunc of lengthFunctions) {
              it(`should work with .to.have.${lenFunc}`, () => {
      ...
      for (const { obj, operator, len } of positiveSuccessCasesWithOperator) {
        describe(`with object ${JSON.stringify(obj)} and operator "${operator}"`, () => {
          for (const convert of [
            ...
            const length = convert(len);
            describe(`with an operand of type ${typestr(length)}`, () => {
              for (const lenFunc of lengthFunctions) {
                it(`should work with .to.have.${lenFunc}.${operator}`, () => {
      ...
      for (const { obj, operator, len } of negativeSuccessCasesWithOperator) {
        describe(`with object ${JSON.stringify(obj)} and operator "${operator}"`, () => {
          for (const convert of [
            ...
            describe(`with an operand of type ${typestr(length)}`, () => {
              for (const lenFunc of lengthFunctions) {
                it(`should work with .not.to.have.${lenFunc}.${operator}`, () => {
    ...
    describe("positive, failing assertions chained off of length should throw the proper error message", () => {
      for (const {
        obj,
        operator,
        len,
  ...
  describe("with two arguments", () => {
    ...
    for (const { operator, operands } of positiveSuccessCases) {
      describe(`.to.${operator}`, () => {
        checkAll(operands[0], operands[1], (a, b) => {
          it(`should work with ${typestr(a)} and ${typestr(b)}`, () => {
    ...
    for (const { operator, operands, msg } of positiveFailureCases) {
      describe(`.to.${operator} should throw the proper message on failure`, () => {
        checkAll(operands[0], operands[1], (a, b) => {
          it(`with ${typestr(a)} and ${typestr(b)}`, () => {
    ...
    operators.forEach((operator: Operator) => {
      describe("should throw when comparing to a non-integral floating point literal", () => {
        for (const convert of numberToBigNumberConversions) {
          const converted = convert(1);
          const msg = "1.1 is not an integer";
      ...
      describe("should throw when comparing to an unsafe integer", () => {
        const unsafeInt = 1e16;
        const msg = `Integer 10000000000000000 is unsafe. Consider using ${unsafeInt}n instead. For more details, see https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Number/isSafeInteger`;
  ...
  describe("with three arguments", () => {
    ...
    for (const { operator, operands, msg } of positiveFailureCases) {
      describe(`.to.be.${operator} should throw the proper message on failure`, () => {
        checkAll(operands[0], operands[1], operands[2], (a, b, c) => {
          it(`with ${typestr(a)}, ${typestr(b)} and ${typestr(c)}`, () => {
    ...
    for (const closeToOrApproximately of closeToAndApproximately) {
      describe(`${closeToOrApproximately} with an undefined delta argument`, () => {
        for (const convert of [
          (n: number) => n,
          ...numberToBigNumberConversions,
        ]) {
          ...
          it(`with a ${typestr(
            one,
          )} actual should throw a helpful error message`, () => {
            expect(() =>
    ...
    for (const { operator, operands, msg } of negativeFailureCases) {
      describe(`.not.to.be.${operator} should throw the proper message on failure`, () => {
        checkAll(operands[0], operands[1], operands[2], (a, b, c) => {
          it(`with ${typestr(a)}, ${typestr(b)} and ${typestr(c)}`, () => {
