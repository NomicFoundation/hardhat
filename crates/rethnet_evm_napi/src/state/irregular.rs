...
use rethnet_evm::{state::StateOverride, AccountInfo, StorageSlot};
...
pub struct IrregularState {
    inner: Arc<RwLock<rethnet_evm::state::IrregularState>>,
}

impl IrregularState {
    pub(crate) fn as_inner(&self) -> &Arc<RwLock<rethnet_evm::state::IrregularState>> {
        &self.inner
    }
}
...
impl IrregularState {
    ...
    pub fn new() -> Self {
        Self {
            inner: Arc::new(RwLock::new(rethnet_evm::state::IrregularState::default())),
        }
    }
