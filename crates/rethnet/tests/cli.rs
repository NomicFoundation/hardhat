...
async fn node() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let method_invocations = [
        MethodInvocation::NonCacheable(Accounts()),
        MethodInvocation::NonCacheable(BlockNumber()),
        MethodInvocation::Cacheable(ChainId()),
        MethodInvocation::NonCacheable(Coinbase()),
        MethodInvocation::NonCacheable(EvmIncreaseTime(U256OrUsize::U256(U256::from(12345)))),
        MethodInvocation::NonCacheable(EvmSetNextBlockTimestamp(U256OrUsize::U256(U256::from(
            12345,
        )))),
        MethodInvocation::Cacheable(GetBalance(address, Some(BlockSpec::latest()))),
        MethodInvocation::Cacheable(GetCode(address, Some(BlockSpec::latest()))),
        MethodInvocation::NonCacheable(GetFilterChanges(U256::from(1))),
        MethodInvocation::Cacheable(GetFilterLogs(U256::from(1))),
        MethodInvocation::Cacheable(GetStorageAt(address, U256::ZERO, Some(BlockSpec::latest()))),
        MethodInvocation::Cacheable(GetTransactionCount(address, Some(BlockSpec::latest()))),
        MethodInvocation::NonCacheable(NetListening()),
        MethodInvocation::NonCacheable(NetPeerCount()),
        MethodInvocation::Cacheable(NetVersion()),
        MethodInvocation::NonCacheable(NewPendingTransactionFilter()),
        MethodInvocation::NonCacheable(UninstallFilter(U256::from(1))),
        MethodInvocation::NonCacheable(Unsubscribe(U256::from(1))),
        MethodInvocation::NonCacheable(Unsubscribe(U256::from(1))),
        MethodInvocation::NonCacheable(Sign(
            address,
            bytes::Bytes::from(hex::decode("deadbeef").unwrap()).into(),
        )),
        MethodInvocation::NonCacheable(Web3ClientVersion()),
        MethodInvocation::NonCacheable(Web3Sha3(Bytes::from_static(b"").into())),
    ...
    for method_invocation in method_invocations {
        Assert::new(output.clone()).stdout(contains(match method_invocation {
            MethodInvocation::NonCacheable(Accounts()) => String::from("eth_accounts()"),
            MethodInvocation::NonCacheable(BlockNumber()) => String::from("eth_blockNumber()"),
            MethodInvocation::Cacheable(ChainId()) => String::from("eth_chainId()"),
            MethodInvocation::NonCacheable(Coinbase()) => String::from("eth_coinbase()"),
            MethodInvocation::NonCacheable(EvmIncreaseTime(increment)) => {
                format!("evm_increaseTime({increment:?})")
            }
            MethodInvocation::NonCacheable(EvmSetNextBlockTimestamp(timestamp)) => {
                format!("evm_setNextBlockTimestamp({timestamp:?})")
            }
            MethodInvocation::Cacheable(GetBalance(address, block_spec)) => {
                format!("eth_getBalance({address:?}, {block_spec:?})")
            }
            MethodInvocation::Cacheable(GetCode(address, block_spec)) => {
                format!("eth_getCode({address:?}, {block_spec:?})")
            }
            MethodInvocation::NonCacheable(GetFilterChanges(filter_id)) => {
                format!("eth_getFilterChanges({filter_id:?})")
            }
            MethodInvocation::Cacheable(GetFilterLogs(filter_id)) => {
                format!("eth_getFilterLogs({filter_id:?})")
            }
            MethodInvocation::Cacheable(GetStorageAt(address, position, block_spec)) => {
                format!("eth_getStorageAt({address:?}, {position:?}, {block_spec:?})")
            }
            MethodInvocation::Cacheable(GetTransactionCount(address, block_spec)) => {
                format!("eth_getTransactionCount({address:?}, {block_spec:?})")
            }
            MethodInvocation::NonCacheable(NetListening()) => String::from("net_listening()"),
            MethodInvocation::NonCacheable(NetPeerCount()) => String::from("net_peerCount()"),
            MethodInvocation::Cacheable(NetVersion()) => String::from("net_version()"),
            MethodInvocation::NonCacheable(NewPendingTransactionFilter()) => {
                String::from("eth_newPendingTransactionFilter()")
            }
            MethodInvocation::NonCacheable(UninstallFilter(filter_id)) => {
                format!("eth_uninstallFilter({filter_id:?})")
            }
            MethodInvocation::NonCacheable(Unsubscribe(filter_id)) => {
                format!("eth_unsubscribe({filter_id:?})")
            }
            MethodInvocation::NonCacheable(Sign(address, message)) => {
                format!("eth_sign({address:?}, {message:?})")
            }
            MethodInvocation::NonCacheable(Web3ClientVersion()) => {
                String::from("web3_clientVersion()")
            }
            MethodInvocation::NonCacheable(Web3Sha3(message)) => {
                format!("web3_sha3({message:?})")
            }
            MethodInvocation::Hardhat(ImpersonateAccount(address)) => {
                format!("hardhat_impersonateAccount({address:?}")
            }
            MethodInvocation::Hardhat(SetBalance(address, balance)) => {
                format!("hardhat_setBalance({address:?}, {balance:?}")
            }
            MethodInvocation::Hardhat(SetCode(address, code)) => {
                format!("hardhat_setCode({address:?}, {code:?}")
            }
            MethodInvocation::Hardhat(SetNonce(address, nonce)) => {
                format!("hardhat_setNonce({address:?}, {nonce:?}")
            }
            MethodInvocation::Hardhat(SetStorageAt(address, position, value)) => {
                format!("hardhat_setStorageAt({address:?}, {position:?}, {value:?}")
            }
            MethodInvocation::Hardhat(StopImpersonatingAccount(address)) => {
