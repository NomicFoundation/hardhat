...
use edr_evm::{
    ...
    mine_block,
    state::{IrregularState, StateDiff, StateError, StateOverride, SyncState},
    Account, AccountInfo, AccountStatus, Block, CfgEnv, HashMap, HashSet, MemPool, MineBlockResult,
    MineOrdering, RandomHashGenerator, SyncBlock, KECCAK_EMPTY,
};
...
async fn create_blockchain_and_state(
    ...
    if let Some(fork_config) = &config.rpc_hardhat_network_config.forking {
        ...
        let blockchain = ForkedBlockchain::new(
            ...
            Arc::new(parking_lot::Mutex::new(state_root_generator)),
