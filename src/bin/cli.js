...
const { getConfig, getUserConfigPath } = require("../core/config");
...
function supportsEmoji(builderArguments) {
  return process.stdout.isTTY && (process.platform === 'darwin' || process.argv.includes("--emoji"));
}
...
async function initProject() {
  ...
  console.log(chalk.blue('88888b.  888  888 888  .d88888 888  .d88b.  888d888'));
  ...
  console.log(chalk.cyan(`  Welcome to ${packageInfo.name} v${packageInfo.version}  \n`));

  const {shouldCreateProject} = await inquirer.prompt([{
    name: "shouldCreateProject",
    type: "confirm",
    message: "You are not inside a buidler project. Do you want to create a new one?"
  }]);
  ...
  await fs.outputFile(path.join(projectRoot, "buidler-config.js"),
  `
module.exports = {
};
  `);
...
async function main() {
  ...
  try {
    if (!insideProject() && process.stdout.isTTY) {
      await initProject();
      return;
    }
