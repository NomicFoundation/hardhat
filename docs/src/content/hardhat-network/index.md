...
You can see an example in the Sample Project. Follow the steps in [Quick Start](/getting-started#quick-start) to try it out. You can also refer to more details in [the reference documentation](./reference#console-log).
...
This logging is enabled by default when using Hardhat Network's node (i.e. `npx hardhat node`), but disabled when using the in-process Hardhat Network provider. See [Hardhat Network's config](../config#hardhat-network) to learn more about how to control its logging.
...
For more details, see [the reference documentation for this method](./reference#debug-tracetransaction).

## Dig deeper

This has been just a high-level explanation of what Hardhat Network is. To dig deeper, see [the Reference documentation](./reference). <!-- TODO: when they exist, add additional links here to "Guides", "Tutorials" and "Explanations" of more specific things. -->
