...
In our tests we're going to use [ethers.js](https://docs.ethers.io/v5/) to interact with the Ethereum contract we built in the previous section, and [Mocha](https://mochajs.org/) as our test runner.
...
You can solve these problems by using **fixtures**. A fixture is a setup function that is run only the first time. When it's invoked again, instead of re-running it, Hardhat will reset the state of the network to the point after the fixture was executed.
