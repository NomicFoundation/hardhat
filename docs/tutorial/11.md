...
- The idea is to go through every aspect of boilerplate's Dapp.js in "coding" order. Maybe divide the content into multiple sections (tbd)
...
- The `Dapp.js`'s code starting point should be the one presented on the previous section + components.
- Through this section we're going to add (a lot of) functionality.
:::
...
- We already have a button inside the `ConnectWallet` component that triggers the `_connectWallet` method, so we're going to write the need logic inside it
...
- We'll create an `_initialize` method which will receive the user's address as parameter. This method will store the address in the Dapp state and initialize the Ethereum Provider
...
- We are going to use `ethers` as our Ethereum provider
...
- It's important to know that sending a transaction is a complex operation:
...
::: tip
Note that some testing networks, like Buidler EVM, do mine transactions immediately, but your Dapp should be prepared for other networks.
:::
